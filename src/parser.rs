  ...
  fn parse<'a>(&self, input: &'a str) -> ParseResult<'a, Self::Result> {
    match input.as_bytes().get(0) {
      Some(x) => {
        let ch = *x as char;
        if ch == self.0 {
          Ok(ParseOk { result: ch, input: &input[1..] })
        } else {
  ...
  fn parse<'b>(&self, input: &'b str) -> ParseResult<'b, Self::Result> {
    match self.0.parse(input) {
      Ok(ParseOk { result, input }) => {
        Ok(ParseOk::new(Either::Left(result), input))
      },
      _ => match self.1.parse(input) {
        Ok(ParseOk { result, input }) => {
          Ok(ParseOk::new(Either::Right(result), input))
        },
        Err(err) => Err(err)
      }
    }
...
impl Parse for WhitespaceParser {
  ...
  fn parse<'a>(&self, mut input: &'a str) -> ParseResult<'a, Self::Result> {
    let mut ws = String::new();
      while let Ok(ParseOk { result, input: input1 })
...
impl<'b, T: Parse> Parse for SepByRef<'b, T> {
  ...
  fn parse<'a>(&self, input: &'a str) -> ParseResult<'a, Self::Result> {
    self.0.sbws_ref().keep_ref(&CharParser(self.1).sbws()).zero_or_more_ref()
      .and(self.0.sbws_ref())
      .map(|(mut v, e)| { v.push(e); v })
      .parse(input)
  }
}
...
pub fn uint_p() -> impl Parse<Result=u64> {
  digit()
    .one_or_more()
    .map(|x| x.into_iter().collect::<String>().parse().expect("This has to be an int"))
}
